...
describe('Posts Service', () => {
  ...
  describe('getPostById', () => {
    ...
    it('存在しないIDの場合、エラーをスローする', async () => {
      await expect(getPostById(999, env)).rejects.toThrow("No post with id '999'");
    });
  });
  ...
  describe('getAllPostsByUserId', () => {
    ...
    it('ページネーションが正しく動作する', async () => {
      ...
      const page1 = await getAllPostsByUserId(testUserId, undefined, env, 1, 10);
      const page2 = await getAllPostsByUserId(testUserId, undefined, env, 2, 10);
  ...
  describe('deletePostById', () => {
    it('投稿を削除できる', async () => {
      ...
      // 削除されたことを確認
      const deleted = await db.select().from(posts).where(eq(posts.id, inserted.id)).get();
